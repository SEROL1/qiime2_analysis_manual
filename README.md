...
もし**誤ったフォルダを作成してしまった場合**や、**解析をやり直したい場合**は**以下のコードで必要のないフォルダのみを削除**することができます。

□□□をフォルダの名前に変更し実行してください。
...
この解析にはデータの数にもよりますが**10分以上**時間がかかることが予想されます。
...
α多様性解析は→**飽和深度の確認→解析→結果可視化→統計解析**


β多様性解析は→**解析→結果可視化→統計解析**

で進むため、👉 https://view.qiime2.org で見るのは

可視化後は確認程度で、**統計解析後に解析結果**　がいいと思います。
...
| 指標       | 意味               |
| -------- | ---------------- |
| Shannon  | 菌の種類数＋均等性をまとめた指標 |
| Faith PD | 系統関係を考慮した多様性     |

この2つは **α多様性の代表的な指標**のため、
基本的な解析では この**2つを中心に結果を判断**します。

以下の指標は **補足的な確認**として使用します

| 指標                | 意味      |
| ----------------- | ------- |
| Observed features | 菌の種類数のみ |
| Evenness          | 菌の均等性のみ |
...
この段階も**20分～数時間とかなり時間がかかることが予想されます**ので注意してください。
...
tmux new -s picrust2 \
  "bash -lc '
    export master=\"$master\"
    export THREADS=1
    bash ~/qiime/tools/run_picrust2.sh
  '" \
  \; attach
```
