...
**🧬 table.qzv（ASV 出現数テーブル）**

**目的：サンプル間のリード数分布を確認する**

確認できること

・各サンプルのリード数（棒グラフ）

・フィルタリング後に十分なリード数が残っているか
→ 例：全サンプルで 10,000 reads 以上が望ましい

・欠損サンプル（リード数ゼロや極端に少ないサンプル）がないか

**次の用途**

・この .qza は α多様性・β多様性解析（core-metrics） のベースになります。
...
・各ASVの配列長分布（Expected length ≈ 400–460 bp ならOK）
...
・この .qza は タキソノミー分類（分類ステップ） に使用します。

**📈 denoising-stats.qzv（デノイズ統計）**

**目的：フィルタリング・マージ・キメラ除去の各段階での除去率を確認**

確認できること

・input → filtered → denoised → merged → non-chimeric のリード数変化

・どの段階でどれだけリードが失われたか

・マージ率（forward/reverse overlap の成功率）やキメラ除去率

**判定目安**

・filtered / input：60〜90% 程度が理想

・merged / denoised：70%以上あれば安定

・特定サンプルだけ異常に低い場合 → トリミング長の再調整を検討

**次の用途**

・品質確認のみ（以降の解析では使用しません）。
